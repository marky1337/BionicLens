...
class DetectedFaceGraphic internal constructor(
    ...
    override fun draw(canvas: Canvas) {
        ...
        var yLabelOffset: Float = if (face.trackingId == null) 0.0f else -lineHeight

        // Decide color based on face ID
        val colorID =
            if (face.trackingId == null) 0 else Math.abs(face.trackingId % NUM_COLORS)

        // Calculate width and height of label box
        var textWidth =
            idPaints[colorID]!!.measureText("ID: " + face.trackingId)
        ...
        textWidth = Math.max(
            ...
            idPaints[colorID]!!.measureText(
                String.format(
                    Locale.US,
                    "X Angle: %d",
                    Math.round(face.headEulerAngleX)
                )
            )
        )
        textWidth = Math.max(
            ...
            idPaints[colorID]!!.measureText(
                String.format(
                    Locale.US,
                    "Y Angle: %d",
                    Math.round(face.headEulerAngleY)
                )
            )
        )
        textWidth = Math.max(
            ...
            idPaints[colorID]!!.measureText(
                String.format(
                    Locale.US,
                    "Z Angle: %d",
                    Math.round(face.headEulerAngleZ)
                )
            )
        )
        // Draw labels
        ...
        if (leftEye != null) {
            val leftEyeLeft =
                translateX(leftEye.position.x) - idPaints[colorID]!!
        ...
        if (rightEye != null) {
            val rightEyeLeft =
                translateX(rightEye.position.x) - idPaints[colorID]!!
        ...
        canvas.drawText(
            "X Angle: " + String.format(
                Locale.US,
                "%d",
                Math.round(face.headEulerAngleX)
            ), left, top + yLabelOffset, idPaints[colorID]!!
        )
        yLabelOffset += lineHeight
        canvas.drawText(
            "Y Angle: " + String.format(
                Locale.US,
                "%d",
                Math.round(face.headEulerAngleY)
            ), left, top + yLabelOffset, idPaints[colorID]!!
